...
function setUnauthorized(httpResponse, rejection){
    var strings = new Strings('en'),
        response = new Response(httpResponse),
        err;

    err = new Error((rejection && rejection.message) || strings.group('errors').unauthorized);
    err.errorCode =
        (rejection && (rejection.code || rejection.errorCode)) || Response.prototype.statusCodes.unauthorized;
